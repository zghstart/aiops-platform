...
#### 3.7.1 版本控制策略
- **URL路径版本**: 使用`/api/v1/`格式在URL中指定版本
- **向后兼容**: 新版本应兼容旧版本的请求格式
- **废弃策略**: 旧版本在新版本发布后保留至少6个月
...
#### 3.8.1 命名规范
- **资源命名**: 使用小写复数形式（如`/api/v1/alerts`）
- **参数命名**: 使用小写蛇形命名法（如`tenant_id`）
- **响应字段**: 使用驼峰命名法（如`incidentId`）
